...
bool trail_layer_update(TrailLayer *trail, SDL_Renderer *renderer, const PreparedRodLine *rod_lines, float delta_time) {
    if(!trail || !renderer || !rod_lines || trail->pendulum_count <= 0 || trail->bucket_count <= 0) {
        return false;
    }
    ...
    for(int i = 0; i < trail->pendulum_count; ++i) {
        const PreparedRodLine *tip_rod = &rod_lines[i * ROD_LINES_PER_PENDULUM + 1];
        SDL_FPoint tip = { (float)tip_rod->x1, (float)tip_rod->y1 };

        if(trail->has_last[i]) {
            if(!line_batch_add(&trail->line_batch, trail->last_tip[i].x, trail->last_tip[i].y, tip.x, tip.y,
                               TRAIL_WIDTH_PIXELS, tip_rod->color)) {
