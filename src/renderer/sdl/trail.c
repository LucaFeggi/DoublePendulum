...
bool trail_layer_init(TrailLayer* trail, int pendulum_count) {
  ...
  trail->buckets =
      (TrailBucket*)calloc((size_t)trail->bucket_count, sizeof(TrailBucket));
  trail->last_tip =
      (SDL_FPoint*)calloc((size_t)pendulum_count, sizeof(SDL_FPoint));
  trail->has_last = (bool*)calloc((size_t)pendulum_count, sizeof(bool));
  if(!trail->buckets || !trail->last_tip || !trail->has_last ||
     !line_batch_init(&trail->line_batch, pendulum_count)) {
...
static SDL_Texture* trail_layer_create_texture(SDL_Renderer* renderer, int w,
                                               int h) {
...
static bool trail_layer_clear_target(SDL_Renderer* renderer,
                                     SDL_Texture* texture) {
...
static void trail_layer_destroy_textures(SDL_Texture** textures,
                                         int texture_count) {
  if(!textures) {
    return;
  }
...
static bool trail_layer_recreate_textures(TrailLayer* trail,
                                          SDL_Renderer* renderer, int w,
                                          int h) {
  SDL_Texture** new_textures =
      (SDL_Texture**)calloc((size_t)trail->bucket_count, sizeof(SDL_Texture*));
  if(!new_textures) {
    return false;
  }
...
static bool trail_layer_ensure_size(TrailLayer* trail, SDL_Renderer* renderer,
                                    int w, int h) {
...
static bool trail_layer_clear_bucket(TrailLayer* trail, SDL_Renderer* renderer,
  ...
  SDL_Texture* old_target = SDL_GetRenderTarget(renderer);
  bool success =
      trail_layer_clear_target(renderer, trail->buckets[bucket_index].texture);
  SDL_SetRenderTarget(renderer, old_target);
  return success;
}

static bool trail_layer_clear_all_buckets(TrailLayer* trail,
...
static bool trail_layer_advance_time(TrailLayer* trail, SDL_Renderer* renderer,
                                     float delta_time) {
  if(delta_time <= 0.0f) {
    return true;
  }
...
bool trail_layer_update(TrailLayer* trail, SDL_Renderer* renderer,
  ...
  for(int i = 0; i < trail->pendulum_count; ++i) {
    ...
    if(trail->has_last[i]) {
      if(!line_batch_add(&trail->line_batch, trail->last_tip[i].x,
                         trail->last_tip[i].y, tip.x, tip.y, TRAIL_WIDTH_PIXELS,
                         tip_rod->color)) {
...
void trail_layer_draw(const TrailLayer* trail, SDL_Renderer* renderer) {
  ...
  for(int offset = trail->bucket_count - 1; offset >= 0; --offset) {
    ...
    float age_seconds =
        (float)offset * TRAIL_BUCKET_SECONDS + trail->bucket_timer;
