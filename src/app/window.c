...
bool window_init(Window *window) {
    ...
    window->ptr = SDL_CreateWindow("Double pendulum | Simulation steps/s: 0 | Renderer FPS: 0 |",
...
void window_update_title(Window *window, double delta_time, double render_fps, double sim_steps_per_second) {
    ...
    if(window->title_timer >= 1.0) {
        char title[80];
        snprintf(title, sizeof(title), "Double pendulum | Simulation steps/s: %d | Renderer FPS: %d |",
                 (int)(sim_steps_per_second + 0.5), (int)(render_fps + 0.5));
